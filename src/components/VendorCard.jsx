...
//   const imageAlt = image && typeof image === "object" ? image.alt : name;
...
const VendorCard = ({ vendor }) => {
  return (
    <div className='bg-white rounded-[2rem] overflow-hidden shadow-[0_15px_30px_rgba(0,0,0,0.02)] border border-gray-100 p-5 flex flex-col justify-between hover:shadow-[0_15px_30px_rgba(147,51,234,0.05)] transition-all duration-300'>
      <div>
        {/* Vendor Image */}
        ...
        {/* Vendor Name */}
        <h3 className='text-lg font-bold text-gray-900 mb-6'>{vendor.name}</h3>
      </div>

      {/* 2. Changed from a <button> to a <Link> pointing to /signup */}
      <Link
        to='/signup'
        className='w-full bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold py-3.5 rounded-xl transition-colors text-center block'
      >
        View Profile
      </Link>
    </div>
